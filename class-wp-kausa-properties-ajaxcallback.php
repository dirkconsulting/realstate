...
class Wp_Kausa_Properties_AjaxCallback {
    ...
    public static function ajaxCallbackLoads() {
        ...
        add_action('wp_ajax_kausa_approve_deny_sale_admin', [__CLASS__, 'kausa_approve_deny_sale_admin']);
    ...
    /**
     * AJAX callback function to mark a property as sold (pending approval) by an agency.
     */
    public static function sold_property_callback() {
        ...
        if (!$reserved_by_user || $reserved_by_user != $user_id) {
            wp_send_json_error(array('message' => 'You cannot sell a property you have not reserved.'));
        }
        ...
        // Buscar el registro en la tabla wp_kausa_properties_meta
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id FROM $table_name WHERE property_id = %d AND property_reserve_time = %s",
                $property_id,
        ...
        if ($row) {
            // Actualizar estado a "pending" en la tabla
            $updated = $wpdb->update(
                $table_name,
                array(
                    'property_status' => 'pending', // Nuevo estado: Venta pendiente de aprobación
            ...
            if ($updated === false) {
                wp_send_json_error(array('message' => 'Failed to update property status in the database.'));
            }

            // Actualizar metadatos
            update_post_meta($property_id, '_kausa_property_sale_status', 'pending');
            update_post_meta($property_id, '_kausa_property_sold', 'pending');
            update_post_meta($property_id, '_kausa_property_reserved', 'no'); // Ya no está reservada, está en proceso de venta
            ...
            update_post_meta($property_id, '_kausa_property_sale_requested_at', $current_time);

            self::trigger_send_email($property_id, $user_id, "pending", "agency", $current_time, $time_spent_formatted);

            wp_send_json_success(array(
                'message' => 'Property sale submitted for approval.',
                'time_spent' => $time_spent_formatted,
                'property_sold_time' => $current_time,
            ));
        } else {
            wp_send_json_error(array('message' => 'No property found to mark as sold.'));
        }
    }

    /**
     * AJAX callback function for admin to approve or deny a sale.
     */
    public static function kausa_approve_deny_sale_admin() {
        check_ajax_referer('kausa_property_status_nonce', 'nonce');

        if (!isset($_POST['property_id']) || !isset($_POST['user_id']) || !isset($_POST['sale_status'])) {
            wp_send_json_error(array('message' => 'Invalid input. Data received: ' . print_r($_POST, true)));
            return;
        }

        $property_id = intval($_POST['property_id']);
        $user_id = intval($_POST['user_id']);
        $sale_status = sanitize_text_field($_POST['sale_status']);
        $current_time = isset($_POST['property_date']) ? sanitize_text_field($_POST['property_date']) : current_time('mysql');

        if (!in_array($sale_status, array('confirmed', 'denied', 'pending'))) {
            wp_send_json_error(array('message' => 'Invalid sale status.'));
            return;
        }
        ...
        error_log('AJAX Request: property_id=' . $property_id . ', user_id=' . $user_id . ', sale_status=' . $sale_status . ', property_date=' . $current_time);

        // Verificar si existe un registro, si no, crearlo
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id FROM $table_name WHERE property_id = %d AND (user_id = %d OR property_sold_by_user = %d) ORDER BY property_reserve_time DESC LIMIT 1",
                $property_id,
                $user_id,
                $user_id
            )
        ...
        if (!$row) {
            $inserted = $wpdb->insert(
                ...
                array(
                    'user_id' => $user_id,
                    'property_id' => $property_id,
                    'property_status' => $sale_status,
                    'property_reserve_time' => null,
                    'property_sold_time' => ($sale_status === 'confirmed') ? $current_time : null,
                    'property_unreserve_time' => null,
                    'property_spent_time' => null,
                    'property_panelty_time' => null,
                ),
            ...
            if ($inserted === false) {
                error_log('Failed to insert new record: ' . $wpdb->last_error);
                wp_send_json_error(array('message' => 'Failed to create record in the database: ' . $wpdb->last_error));
                return;
            }
        } else {
            $updated = $wpdb->update(
                ...
                array(
                    'property_status' => $sale_status,
                    'property_sold_time' => ($sale_status === 'confirmed') ? $current_time : null,
                    'property_sold_by_user' => ($sale_status === 'confirmed') ? $user_id : null,
                ),
                array('id' => $row->id),
                array('%s', '%s', '%d'),
                array('%d')
            ...
            if ($updated === false && $wpdb->last_error) {
                ...
                wp_send_json_error(array('message' => 'Failed to update sale status in the database: ' . $wpdb->last_error));
                return;
            }
        }

        // Actualizar metadatos
        if ($sale_status === 'confirmed') {
            update_post_meta($property_id, '_kausa_property_sale_status', 'confirmed');
            ...
            update_post_meta($property_id, '_kausa_property_sold_time', $current_time);
            update_post_meta($property_id, '_kausa_property_sold_by_user', $user_id);
        } elseif ($sale_status === 'denied') {
            update_post_meta($property_id, '_kausa_property_sale_status', 'denied');
            update_post_meta($property_id, '_kausa_property_sold', 'no');
            update_post_meta($property_id, '_kausa_property_reserved', 'yes');
            update_post_meta($property_id, '_kausa_property_avaliable_to_reserve', 'no');
        ...
        error_log('Updated property_status to: ' . $sale_status);

        self::trigger_send_email($property_id, $user_id, $sale_status, "admin", $current_time, '00h 00m 00s');

        wp_send_json_success(array(
            'message' => "Property sale $sale_status successfully.",
            'property_status' => $sale_status,
        ));
    }
    ...
    public static function kausa_change_property_status_admin() {
        ...
        if (!isset($_POST['property_id'], $_POST['user_id'], $_POST['sale_status'], $_POST['property_date'])) {
            wp_send_json_error(['message' => 'Invalid input.']);
        }

        $property_id = intval($_POST['property_id']);
        $user_id = intval($_POST['user_id']);
        $property_status = sanitize_text_field($_POST['sale_status']);
        $current_time = sanitize_text_field($_POST['property_date']);

        global $wpdb;
        $table_name = $wpdb->prefix . 'kausa_properties_meta';

        if ($property_status == 'unreserved') {
            $reserved_time = get_post_meta($property_id, '_kausa_property_reserved_time', true);
            $reserved_by_user = get_post_meta($property_id, '_kausa_property_reserved_by_user', true);
            ...
            if ($row) {
                ...
                update_post_meta($property_id, '_kausa_property_reserved_no_penalty_time', null);
                update_post_meta($property_id, '_kausa_property_reserved_penalty_time', null);
                self::trigger_send_email($property_id, $reserved_by_user, "unreserved", "admin user", $current_time, $time_spent_formatted);
                wp_send_json_success(array(
                    'message' => 'Property unreserved successfully.',
                    'time_spent' => $time_spent_formatted,
                    'property_unreserve_time' => $current_time,
                ...
                // Si no hay registro previo, insertamos uno nuevo para "unreserved"
                $inserted = $wpdb->insert(
                    $table_name,
                    array(
                        'user_id'               => $user_id,
                ...
                if ($inserted === false) {
                    wp_send_json_error(array('message' => 'Failed to unreserve the property in the database.'));
                }
                ...
                update_post_meta($property_id, '_kausa_property_reserved_no_penalty_time', null);
                update_post_meta($property_id, '_kausa_property_reserved_penalty_time', null);
                self::trigger_send_email($property_id, $user_id, "unreserved", "admin user", $current_time, "");
                wp_send_json_success(array(
                    'message' => 'Property unreserved successfully.',
                    'time_spent' => '00h 00m 00s',
                    'property_unreserve_time' => $current_time,
                    'penalty_time' => '00h 00m 00s',
                    'penalty_end_time' => '00h 00m 00s',
                ));
            }
        } elseif ($property_status == 'unreserve') {
            // "Desbloquear" es lo mismo que "unreserved" para el admin
            $reserved_time = get_post_meta($property_id, '_kausa_property_reserved_time', true);
            $reserved_by_user = get_post_meta($property_id, '_kausa_property_reserved_by_user', true);
            ...
            self::trigger_send_email($property_id, $user_id, "reserved", "admin user", $current_time, "");
            wp_send_json_success(array(
