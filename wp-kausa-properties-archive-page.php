...
if (!function_exists('get_property_filter_options')) {
    function get_property_filter_options() {
        $property_attributes = array(
            ...
            "Province" => array(
                'meta_key' => 'kausa_property_state',
            ),
            "Commission" => array( // Añadido el filtro de comisión
                'meta_key' => 'kausa_property_commission',
            ),
        );
        ...
        foreach ($property_attributes as $attribute => $details) {
            ...
            if ($attribute === "Price" || $attribute === "Commission") { // Tratar Comisión como rango numérico
                $values = $wpdb->get_col(
                    $wpdb->prepare(
                        "SELECT DISTINCT CAST(meta_value AS DECIMAL(10,2)) AS value 
                        FROM {$wpdb->postmeta} 
                        WHERE meta_key = %s",
                        $meta_key
                    )
                ...
                if (!empty($values)) {
                    ...
                    $filters[$attribute] = [
                        'meta_key' => $meta_key,
                        'min' => 0,
                        'max' => 10000, // Ajusta el máximo según tus necesidades
                    ];
                }
            } else {
                $values = $wpdb->get_col(
    ...
    <div class="kausa-properties-archive-container-wrapper">
        ...
        <div class="kausa-properties-filter-sidebar">
            ...
            $property_filter_heading_array = array(
                ...
                'Province' => isset($property_filter_heading['kausa-property-state']) ? $property_filter_heading['kausa-property-state'] : "Provincia",
                'Commission' => isset($property_filter_heading['kausa-property-commission']) ? $property_filter_heading['kausa-property-commission'] : "Comisión",
            );
            foreach ($filters as $attribute => $data) {
                ...
                if ($attribute === 'Price' || $attribute === 'Commission') {
                ...
                if ($attribute === 'Price' || $attribute === 'Commission') {
                    ...
                    $min_value = (float) $rounded_bounds['min'];
                    $max_value = (float) $rounded_bounds['max'];

                    echo '<div class="property-attribute-label property-attribute-price-box">';
                    echo '<input type="text" id="' . esc_attr(strtolower(str_replace(' ', '-', $attribute))) . '-range-slider" class="js-range-slider" name="' . esc_attr(strtolower(str_replace(' ', '_', $attribute))) . '_range" value="" ';
                    echo 'data-skin="round" data-type="double" data-min="' . esc_attr($min_value) . '" ';
                    echo 'data-max="' . esc_attr($max_value) . '" data-grid="false" />';
                    echo '<input type="hidden" id="' . esc_attr(strtolower(str_replace(' ', '-', $attribute))) . '-range-min" value="' . esc_attr($min_value) . '" class="from" />';
                    echo '<input type="hidden" id="' . esc_attr(strtolower(str_replace(' ', '-', $attribute))) . '-range-max" value="' . esc_attr($max_value) . '" class="to" />';
                    echo '<span id="' . esc_attr(strtolower(str_replace(' ', '-', $attribute))) . '-range-value" style="display:none;">' . esc_html($min_value) . '€ - ' . esc_html($max_value) . '€</span>';
                    echo '</div>';
                } else {
        ...
        <div class="kausa-properties-listing-outer">
            <div class="kausa-properties-listing">
                ...
                if (!empty($_GET['province'])) {
                    $kausa_args['meta_query'][] = array(
                        'key'     => 'kausa_property_state',
                ...
                // Filtro por comisión
                if (!empty($_GET['commission_range'])) {
                    list($min_commission, $max_commission) = explode('-', sanitize_text_field($_GET['commission_range']));
                    $kausa_args['meta_query'][] = array(
                        'key'     => 'kausa_property_commission',
                        'value'   => array(floatval($min_commission), floatval($max_commission)),
                        'compare' => 'BETWEEN',
                        'type'    => 'DECIMAL'
                    );
                }
                ...
                if ($kausa_properties->have_posts()) {
                    while ($kausa_properties->have_posts()) {
                        ...
                        $commission = get_post_meta(get_the_ID(), 'kausa_property_commission', true);
                            ...
                            <a href="<?php the_permalink(); ?>" class="kausa-property-item-wrapper">
                                ...
                                <div class="kausa-property-details">
                                    ...
                                    <?php if (!empty($commission)) { ?>
                                        <div class="kausa-property-commission-wrapper">
                                            <p class="kausa-property-commission">Comisión: <?php echo esc_html(number_format($commission, 2, ',', '.')); ?>€</p>
                                        </div>
                                    <?php } ?>
                                    <?php
